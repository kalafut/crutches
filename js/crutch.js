...
(function() {
  var contains, filter, hide, matches, parse_search, search_split, some;
  ...
  matches = function(needle, haystack) {
    ...
    match = false;
    if (needle[0] !== "^") {
      return match = haystack.indexOf(needle) !== -1;
    } else {
      idx = haystack.indexOf(needle.substring(1));
      if (idx !== -1 && (idx === 0 || haystack[idx - 1] === " ")) {
        return match = true;
      }
    }
  };
  ...
  filter = function(query) {
    ...
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      ...
      if (!contains(active_project_uid, project.uid) || (query.projects.length > 0 && some(query.projects, function(search) {
        return !matches(search, project.project);
      }))) {
      ...
      while (section_idx < project["sections"].length) {
        ...
        if (query.sections.length > 0 && some(query.sections, function(search) {
          return !matches(search, project.sections[section_idx].section);
        })) {
        ...
        while (entry_idx < project["sections"][section_idx]["entries"].length) {
          ...
          if (project_disabled || section_disabled || query.terms.length > 0 && some(query.terms, function(exp) {
            var not_found;
            not_found = entry.term != null ? !matches(exp, entry.description) && !matches(exp, entry.term) : !matches(exp, entry.description);
            return not_found = not_found && !matches(exp, project.project) && !matches(exp, project.sections[section_idx].section);
          })) {
            matched = false;
          }
          if (matched) {
